...
{
curl -s -F chat_id=$chat_id -F document=@$resource_usage_info -F caption="$caption" https://api.telegram.org/bot"yourtoken"/sendDocument > /dev/null 2&>1
}
...
do
    ...
    if [ $min_load -ge $load_threshold ]
        then
        echo -e "Sir High CPU usage detected on $(hostname)\n$(uptime)" > $msg_caption
        echo -e "Sir CPU usage report from $(hostname)\nServer Time : $(date +"%d%b%Y %T")\n\n\$uptime\n$(uptime)\n\n%CPU %MEM USER\tCMD" >         $resource_usage_info
...
#Monitoring Memory usage on the server
    ...
    then
        echo -e "Sir High Memory usage detected on $(hostname)\n$(echo $mem_usage% memory usage)" > $msg_caption
        echo -e "Sir Memory consumption Report from $(hostname)\nServer Time : $(date +"%d%b%Y %T")\n\n\$free -m output\n$mem\n\n%MEM %CPU USER\tCMD" > $resource_usage_info
